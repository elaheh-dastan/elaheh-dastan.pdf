...
\cventry{Jul 2024 -- present}{Senior Data Scientist}{Digikala}{Tehran, Iran}{}{
      \begin{itemize}[label=\textbullet]
        \item Used \textbf{Contrastive Learning} and \textbf{Flag Embedding} techniques to enhance fine-tuning efficiency on semantic models.
        \item Trained a Query Understanding (QU) model leveraging user query logs, click data to prune low-confidence queries and improve semantic alignment.
        \item Improving search relevance via multi-metric fine-tuning and category-consistent retrieval with Category Loss and GMV-weighted balancing.
        \item Designed \textbf{Claude-powered} query variation generation to improve generalization and long-tail recall, leading to measurable gains in semantic model robustness.
        \item Led the \textbf{Exact Match project} to enable precise product code recognition (e.g., "PG32UCD Laptop") by implementing tokenizer modifications, custom masking, and a \textbf{Redis-based code index}, resulting in significant Add-to-Cart improvements.
        \item Investigated \textbf{live feedback loops} using user engagement signals and evaluated \textbf{ANCE (Approximate Nearest Contrastive Learning)} for real-time embedding updates.
        \item Built scalable user segmentation using \textbf{embedding-based}, \textbf{feature-driven}, and \textbf{clustering} techniques; improved conversion rate by \textbf{5\%} through iterative class refinement and premium user targeting.
        \item Compared Semantic BA and Elastic Search pipelines across top queries to guide metric alignment, offline evaluation, and production deployment decisions with an emphasis on quantitative rigor.
        \item Developed an \textbf{LLM chatbot} to guide users through product discovery and selection on Digikala. Integrated \textbf{semantic retrieval} and \textbf{contextual recommendation} pipelines, enabling multi-turn dialogue, personalized suggestions, and seamless handoff between conversation and structured product search.
        \item Built an \textbf{image-to-product retrieval system} that allows users to upload or share any image to find visually similar products on Digikala. Designed and trained a \textbf{dual-encoder model} combining \textbf{CLIP-based visual embeddings} with textual product representations, enabling cross-modal search with high precision and scalability across millions of catalog items.
        \item Integrated an \textbf{object detection pipeline} into the image search system to automatically localize products within uploaded images, improving matching precision and retrieval accuracy by \textbf{3\%}. Leveraged region-based embeddings to isolate relevant features and reduce background noise in visual search results.
      \end{itemize}
...
\cventry{Feb 2024 -- Jul 2024}{Machine Learning Consultant}{Asan Pardakht}{Tehran, Iran}{}{
      \begin{itemize}[label=\textbullet]
        ...
        \item Integrated Monte Carlo simulations for profit expectation under different market scenarios.
        \item Delivered a prototype that outperformed baseline strategies (e.g., equal-weighted portfolio) by 15\% in backtesting.
      \end{itemize}
