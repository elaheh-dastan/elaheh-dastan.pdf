...
\begin{samepage}
\cventry{Jul 2024 -- Dec 2025}{Senior Data Scientist}{Digikala}{Tehran, Iran}{}{
      \begin{itemize}[label=\textbullet]
        \item Accelerated semantic embedding model fine-tuning by \textbf{30\%} using contrastive learning with the \textbf{FlagEmbedding framework}, achieving faster convergence and improved embedding consistency.
        \item Trained a \textbf{Query Understanding (QU)} model leveraging user query logs and click data to infer user intent and reformulate low-confidence queries, reducing zero-click searches by \textbf{4\%}.
        ...
        \item Investigated feedback-driven search optimization using user engagement signals, and evaluated \textbf{ANCE}’s suitability for near-real-time embedding refresh and retraining.
        ...
        \item Extended image search to support \textbf{video input} by building a processing pipeline using \textbf{FFmpeg} for transcoding and \textbf{keyframe extraction}, enabling users to find products from social media clips. Implemented \textbf{scene detection} (PySceneDetect) to isolate product frames, improving retrieval relevance by \textbf{15\%}.
        \item Established \textbf{LLMOps practices} including prompt evaluation harnesses, \textbf{Langfuse} for LLM tracing and observability, and \textbf{token cost monitoring} dashboards integrated with Grafana, reducing LLM inference costs by \textbf{25\%} through prompt optimization.
      \end{itemize}
      \textbf{Skills:} Python, PyTorch, TensorFlow, LLMs, LangGraph, RAG, Vector Database, Retrieval Systems, Vector Search, ElasticSearch, Redis, Data Engineering, Experiment Design, A/B Testing, Statistical Modeling
}
\end{samepage}
